...
function Navbar({ links, isCollapsed, className, closeNav }: NavbarProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "group border-b bg-background transition-[max-height,padding] p-5 duration-500 data-[collapsed=true]:py-2 md:border-none",
        className
      )}
    >
    ...
    <Collapsible defaultOpen={isChildActive}>
      <CollapsibleTrigger
        className={cn(
          ...
          "group h-8 w-full justify-start rounded-none px-6"
        )}
      >
        <div className="mr-2">{icon}</div>
        <span className="text-[0.625rem]">{title}</span>
